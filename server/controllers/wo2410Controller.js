...
const WO2410 = require('../models/wo2410Model');
...
const addWo = async (req, res) => {
    ...
    try {
        const newWo = await WO2410.create({
            id_gudang,
            id_admin_kancab,
            id_alokasi,
            id_provinsi,
...
const updateWO = async (req, res) => {
    ...
    try {
        const [updated] = await WO2410.update({
            id_gudang,
            nomor_wo,
            tanggal_wo,
            qr_wo
        ...
        if (updated) {
            const updatedWo = await WO2410.findByPk(id, {
                include: [
                    {
                        model: Gudang,
                        as: 'gudang',
...
const getDetailsWO = async (req, res) => {
    const { id } = req.params;
    try {
        const wo = await WO2410.findByPk(id, {
            include: [
                {
                    model: Alokasi,
                    as: 'alokasi',
        ...
        wo.item_wo_by_wo_2410.forEach(item => {
