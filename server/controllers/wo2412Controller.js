...
const WO2412 = require('../models/wo2412Model');
...
const addWo = async (req, res) => {
    ...
    try {
        const newWo = await WO2412.create({
            id_gudang,
            id_admin_kancab,
            id_alokasi,
            id_provinsi,
...
const updateWO = async (req, res) => {
    ...
    try {
        const [updated] = await WO2412.update({
            id_gudang,
            nomor_wo,
            tanggal_wo,
            qr_wo
        ...
        if (updated) {
            const updatedWo = await WO2412.findByPk(id, {
                include: [
                    {
                        model: Gudang,
                        as: 'gudang',
...
const getDetailsWO = async (req, res) => {
    const { id } = req.params;
    try {
        const wo = await WO2412.findByPk(id, {
            include: [
                {
                    model: Alokasi,
                    as: 'alokasi',
        ...
        wo.item_wo_by_wo_2412.forEach(item => {
